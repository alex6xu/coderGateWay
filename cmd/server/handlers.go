...
func handleListModels(database *db.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := database.Query(`
			SELECT models, type FROM channels WHERE status = 1 AND user_id = ? ORDER BY priority DESC, weight DESC
		`, accountID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to query channels"})
			return
	...
	return func(c *gin.Context) {
		...
		if sessionID == "" {
			...
			_, err := database.Exec(`
				...
				VALUES (?, ?, ?, 'web', 0, ?, ?)
			`, sessionID, accountID, req.Message[:min(50, len(req.Message))], time.Now(), time.Now())
			if err != nil {
		...
		log.Printf("[chat/agent] account=%d session=%s model=%s channel=%s(type=%d)", accountID, sessionID, modelName, channel.Name, channel.Type)
